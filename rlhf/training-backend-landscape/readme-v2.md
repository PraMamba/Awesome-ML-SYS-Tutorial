...
我读训练框架的源码，最初的动机其实很朴素：我发现自己会背一堆名词，却答不出一个具体的问题。就拿「你们都上 ZeRO-3」这句话来说，它在 DeepSpeed 里意味着一个枚举被设成某个值，在 torchtitan 里意味着给分片原语一个非平凡的 mesh，而在 Megatron-LM 里则要额外打开一条和分布式优化器不同的装填路径。**同一个功能名，在三家的代码里落点完全不同，代价也完全不同。** 我最初以为这是文档写得不好，读下来才发现，问题出在我自己一直在用一张功能矩阵去理解一群目标不同、边界不同的系统。

功能矩阵这种写法有它的用处，我个人并不想否定它：谁支持 ZeRO-3、谁有 DualPipe、谁默认开 FP8，一张表看完，非常适合快速筛掉不合适的候选。但它回答不了我后来真正关心的那个问题，因为**它比较的是名字，不是机制**。举一个我自己被绕进去过的例子：DeepSpeed 的参数预取深度是硬编码在源码里的（旁边还留着一句「以后有空做成可配」的注释），而 AutoModel 的预取是一条用户可开关、深度可调的链；Megatron-LM 的 batch 不变性由六条配置断言把住，torchtitan 的同一件事则要靠十项 NCCL 环境变量换回来。这些差别不会出现在任何功能矩阵里，但它们决定了你在自己的集群上能调什么、不能调什么。

所以这篇文章换一种组织方式：**不问「谁有什么」，而问「同一个设计问题，各家选了哪条路，在这条路上具体怎么跑」。**

具体来说，全文沿十个基本设计决策展开，每个决策都按同一个三段式推进：先归纳出若干种真实存在的方法并给一张分类表，把五个框架归入这些方法；然后逐一进入每个方法，写清采用它的框架各自的执行路径，包括从哪个配置进入、哪些张量或进程组参与、谁触发通信、谁等待结果；最后收拢同一条路上的分岔与代价。我给自己定的验收标准是，**读者读完一个维度应该能连着回答四个问题**：这个问题有哪些做法？哪些框架用哪种做法？同一种做法在各框架里具体怎样运行？它们在什么条件下差别才会显现？

我会按下面四条线走：

1. 先把账算出来，说清单卡为什么装不下，以及「切」与「缝」的代价如何记在显存、通信、算力这三只角上（§动机与共同系统问题）；
2. 交代样本与口径，因为五个框架不是同一种东西，比较的深度天然不同（§比较对象与设计维度）；
3. 逐维度横向拆解，每个维度先给方法分类表，再按方法逐个框架写实现，最后收拢同方法内的分岔（§维度 1 到 §维度 10）；
4. 收敛到判断，包括哪些耦合是硬的、五个框架各自的取舍是什么，以及三组我敢直接拿去用的选型判断（§跨维度关系与设计启示）。

**有两件事我想先说清楚，因为它们决定了你该怎么读后面那些表格。** 一是本文只做机制比较，不做易用性排名：正文里会出现大量「有 / 无」的对比，但「无」不等于差，它往往只是定位不同：库类后端只负责状态与通信这一段，框架类后端替你做了全部决定，拿这两类东西比配置项数量没有意义（这一点我自己在读别的横评时踩过坑，看到「不支持」三个字就下了结论，后来才发现人家根本没打算管那件事）。二是全文的通信量、显存、气泡与重算数字都是**推导值而不是实测值**，它们能给出量级与兑换方向，但回答不了「在你这套集群上哪种配置更快」；数学只出现在解释机制与权衡的地方，并且都标了假设与单位；凡是项目自己宣称的性能数字，我一律标注为「文档声称」，不当事实使用。

本文讲的是「训练池内部」这一侧的成本结构。与之配套的另一篇《让 Token 持续流动：16 个开源 RL 框架的异步训练基础设施横向调研》讲的是「推理池与训练池之间」的关系，也就是生成与训练要等多久、权重怎么送过去、允许训练落后生成多少步。**两篇合起来才是一个 RL 训练系统的完整成本结构**，而它们的优化方向经常冲突，这一点我在最后一节用一个具体例子说明。
...
要看清楚这件事，我想先回到一个最朴素的训练循环。它做的事情少得可怜：取一个 batch，前向算 loss，反向算梯度，优化器更新参数，梯度清零。
...
既然寿命不同，它们占的字节数自然也不同，而把每一类各自的字节数加起来，就得到了一张关于规模的判据。在 AdamW 加混合精度的常规配置下，**每个参数的静态开销是 16 字节**：
...
这笔账一旦算出来，规模上的分界线就立刻显形，而且它会比我预期得早很多到来：一个 7B 模型的光静态状态就是 \(7\times10^9 \times 16\ \mathrm{B} = 104.3\ \mathrm{GiB}\)，装不进一张 80 GB 的卡；671B 模型则是 **9.8 TiB**，即便切到 64 张卡，每卡仍要 156 GiB。换句话说，**单卡装不下这件事，在 7B 这个量级就已经发生了**，而它正是后面所有设计的起点。

**单卡装不下，是全文唯一的出发点。** 其余所有机制都是它的推论：把状态与计算摊到多张卡上（切），再用通信与算力把切口缝起来（缝）。而切与缝都不是免费的，这些开销最后都落在三个地方：**显存、通信、算力**。它们构成一组不可能同时优化的兑换关系：你省下显存，往往要付出通信或算力。所以每个框架的设计差异，在我看来最后都可以还原成同一句话，也就是它把哪一只角让给了另外两只。
...
本文比较的五个项目，都是在大模型训练中实际被当作训练框架用、而且本地有源码可以逐个核对的。它们都能独立跑完训练，但**在「训练循环归谁」这件事上形态不同**，而这一点直接决定了我在后面每个维度里能问出多深的答案：对一个把循环接管过去的框架，我可以追到「它替你决定了什么」；对一个只提供引擎的库，我只能追到「它暴露了哪些原语、边界画在哪」。
...
所以后面的比较里，你会看到两种不同性质的结论并排出现：对 DeepSpeed 这类库，结论的形式是「它提供了哪些原语、边界画在哪」；对 Megatron-LM、torchtitan 这类完整框架，结论的形式是「它替你做了哪些决定、这些决定写在哪一行」。两者的深度天然不同，这不是可比性的缺陷，而是**每条比较结论都必须附带的适用条件**。我自己在读别人的横评时，最怕的就是这一点被省略，因为它会让「某框架不支持」这种话变得毫无意义（不支持什么？在哪个层次上不支持？）。

样本边界我用一句话交代：本文只收**自带完整分布式切分机制、能独立跑完训练**的项目。至于那些只提供可组合分片原语的库，它们是能被框架调用的零件而不是能自己跑的机器，与这五家的抽象层级不同，因此不纳入比较；这个判断本身也影响了我对每一维的提问方式，我在上一段里已经说明。
...
先把五家的源码基线摆出来，因为后面每一条结论都要回到这些 commit 上去复核。我固定的是**上游仓库的 commit**而不是本地 fork 的 HEAD，理由是前者任何人都能拉到；引用 commit 与本地工作树的关系记在文末「引用口径」，正文里出现的每一个路径与行号都可以在对应 commit 上逐条对回去。
...
这五个 commit 就是我在本地工作树上核对源码时用的版本。有一个细节值得写出来，因为它决定了这些行号能不能被信任：除 pith-train 之外，四个仓库的本地工作树各自比引用 commit 多了一个提交，而**那个提交只新增文件、不修改任何已有文件**（Megatron-LM 与 DeepSpeed 各新增一份分析与报告文档，torchtitan 与 AutoModel 各新增一批开发工具配置）。我逐个核对过这一点，所以**正文里的路径与行号在两个版本上完全一致**，你想用哪个核对都行。

### 十个维度分别回答什么

十个维度不是先拍出来再往上凑内容的，划分方式是**先找一个具体的、可执行的设计决策，再问这个决策有哪些真实存在的解法**。我给自己加了一条约束：一个维度只承载一个决策，因此它必须能用一句话问出来，也必须能让每个框架给出一个明确的落点。后面你会看到，凡是没法让某一家给出落点的问题，我就没有把它单列成维度。
...
这张表只用来定位后文，它不替代后文，也不承担结论。**真正有信息量的部分在表后面**：每个维度的方法分类依据与这张表一致，表里列出的每一种方法，在对应维度里都能找到它自己的框架实现分析，包括从哪个配置进入、谁触发通信、谁等待结果。
...
顺着这张网格往下问，就得到本文的第一个设计问题，也就是**这件事由谁负责、在哪一步完成、约束在哪里被检查**。我把它放在维度 1 而不是放到后面，是因为它决定了三件下游的事：配置写错时会在启动就报错还是跑到一半才崩；你在开跑之前能不能算出「我这 512 张卡该怎么摆」；以及**可除性约束被施加在哪一级**：最后这一条最容易被忽略，但它直接决定了哪些并行组合在数学上根本不可行。
...
看这张表的时候要小心一件事：这四种方法不是从低级到高级的四个抽象层级，而是**同一个决策的四种落点**，问的都是「谁持有拓扑、什么时候检查约束」。前三种的差别在工程组织方式上，说白了是同一套数学换了个写法；第四种不一样，它改的是约束本身的可满足集合，所以它是这一维度里唯一一处「数学上不同的解」。
...
读到这里我有一个提醒：**它的字段比它宣传的轴少，这一点在做横向比较时必须区分开**，否则你会把「配置面暴露的轴」当成「机制上支持的轴」，而这两件事在这个框架里差得不小。 torchtitan 常被描述为支持 `DP-replicate / DP-shard / TP / SP / PP / VPP / CP / EP`，而 `ParallelDims` 只直接管六个轴：**SP 是 TP 的子开关**（配置里的 `enable_sequence_parallel`），**VPP 由 `pipeline_parallel_layers_per_stage` 推出**，两者都不在字段里（前者的默认值是开）。另外它的 EP 是从 `dp_shard * cp * tp` 借秩的：`efsdp` 用整除算出，含 CP 而不只是分片维与 TP，上面那条整除校验正是为它准备的。
...
我之前看到「两家都用显式拓扑对象」就以为它们在 CP 上差不多，这个判断是错的：**那句话只说明它们在这一层的组织方式相同**，真正决定接入成本的是字段下面挂着几类入口，以及这些入口要求什么样的 attention backend。
...
这个选择本身是自洽的，我并不觉得它是偷懒：**既然模型定义权完全交给了用户，那么拓扑就不可能是一个由框架推导的对象**，因为框架连你的模型长什么样都不知道。用一句大白话说，它把「该怎么切」这件事连同模型定义一起外包给了你。代价是用户失去了一个可以检查组合合法性的位置。它的 CP 情况尤其能说明这一点：全仓检索 `context_parallel*` 零命中，但它有一个 Ulysses sequence parallelism 实现（`deepspeed/runtime/sequence_parallel/ulysses_sp.py`，配置键 `sequence_parallel_size`）。其 docstring 写明这是「通过序列并行与注意力头并行来高效训练长序列」的机制，**语义上就是头并行的 CP，只是没有被命名为 CP**。按机制它算得上一档 CP，按命名它不算，而这个差别会实际影响用户能否从配置面推断出「我能不能做序列切分」。
...
**同一件事，四家做法的差别到底落在哪？** 把四种方法并排看，分岔集中在三处，而不是「支持多少轴」这种计数：
...
**只有 pith-train 通过折叠让这两条轴互不约束。** 在其余四家，`cp` 与 `ep` 都要和 DP、TP 一起落在同一个整除关系里；而在 pith-train 上，它们各自整除 stage size 就够了。这一条差别看似只是写配置时少算一个数，实际上它决定了「cp=3 配合 ep=5」这类组合能不能存在。

---

## 维度 2：状态的分布与每卡静态开销

维度 1 决定能沿哪些轴切，但它没有回答切了之后到底省多少，这一维度就补上这个缺口。**它的基本设计问题是：参数、梯度与优化器状态这三类状态分别落在哪些设备上，每张卡在两次迭代之间要留下多少静态开销。** 这个决策的结构比上一维度干净得多，因为状态是静态的，切成几份就是几份，中间没有调度与时序来捣乱，所以这一维的数学也是全文最干净的一段。
...
如果用一个出差的场景来记这三档，会容易很多：**不全切相当于所有行李都随身带，切到状态与梯度为止相当于把最占地方的那几箱托运、常用的还背在身上，连参数一起切则是连随身包也托运、用的时候去取。** 这个类比的好处是它立刻解释了后面那个关键区别：托运行李省的是你身上的负担，代价是你每次要用都得先去取一趟。

读这张表的时候还要抓住一个容易看偏的点：**它应该按「什么在迭代之间常驻」来读，而不是按「什么被切了」来读。** 三列的含义分别是：参数常驻指每张卡在两次迭代之间保留多少参数字节；梯度常驻指反向结束后保留多少梯度字节；优化器状态指每张卡持有的 \(m\) 与 \(v\)。之所以强调这个读法，是因为「切」是个动作，而决定显存的是动作之后留下来什么。

既然要按「留下什么」来读，就有必要先排掉一处我自己也犯过的记账错误：**不要把「某个 stage 切了梯度」直接读成「梯度通信变成了 reduce-scatter」**，这两件事在这些框架里是分开的。举一个反例：数据并行完全不切状态时，梯度通信本来就可以用 reduce-scatter 来做，而每张卡依旧会保留完整的梯度缓冲，因为反向还要往它里面累加。反过来，切到状态与梯度那一档在反向之后只留自己那份梯度，靠的是「归约完把非本分区清掉」，也没换归约原语。**换句话说，省显存的是「留什么」，不是「用什么原语」。**
...
把三家放在一起看就会发现，这个方法内部的实现差异几乎全部集中在三个地方：**取回被安排在什么时机、能提前多少、以及能不能不丢**。顺着这三问往下读，你基本能预判每一家会踩到什么坑。
...
**同样一句「开 ZeRO」，五家到底在开什么？** 答案取决于它们各自省下的是哪一份状态，以及那个开关默认是开还是关。
...
维度 2 里每一档省下来的显存都不是白来的：参数被切开之后，用的时候就得先合回来，于是问题从「省了多少字节」自然转成了「这笔合回来的开销记在谁头上」。**这一维度的基本设计问题是：每一次通信在什么时机被发起，它能不能与某一段计算重叠。** 我之所以把它紧跟维度 2，是因为这两维是一组直接的兑换关系，切分与通信的汇率只有在同一处看才不会算错。
...
**那个 \(3P\) 是这一维度最值得推导的数字**，因为它反直觉：为什么参数分片（省显存）会让通信量涨到全复制的 1.5 倍？三个 collective 各干各的一件事：前向算之前必须把参数合回完整的一份（\(P\)）；前向用完若立刻丢掉，反向算梯度时参数又不在手上，必须再合一次（\(P\)）；梯度算出来后要在 DP 组内规约，且规约结果是切片的，reduce-scatter 恰好同时完成规约与切分（\(P\)）。三次相加就是 \(3P\)。用一句大白话说：**你为了省显存把参数拆散，就得在每个需要它的时刻把它重新拼起来，而「拼」这个动作本身要过网络。**

**这就是「通信换显存」的明码标价**：从 \(2P\) 涨到 \(3P\) 多付的那 1.5 倍，换回来的是维度 2 表里 64 倍的显存降幅。至于这笔汇率合不合算，我看下来只有一个判据，也就是你的瓶颈到底在显存还是在带宽上：而这两者谁先到顶，恰恰是不同团队给出不同答案的根本原因。

**TP 是完全不同的一种贵，而且贵的方式不一样。** 上面的表里它写的单位是「每层每前向」，这个形式本身就在提示差别：DP 系的通信是「每步几次」，而 TP 是「每层每前向几次」。一个 32 层模型每前向的 TP 通信量，是层数乘上每层两处 all-reduce；单看量级它比一次 \(P\) 小，可频率高出两三个数量级。**换句话说，DP 系的通信贵在「每次搬多少」，TP 贵在「一年搬多少次」。**

**重叠效果用暴露时间衡量。** 设一次通信需要 \(T_{\mathrm{comm}}\)，可与之重叠的计算有 \(T_{\mathrm{comp}}\)，则真正落在关键路径上的部分是 \(T_{\mathrm{exposed}} = \max(0, T_{\mathrm{comm}} - T_{\mathrm{comp}})\)。这个式子给出两个我觉得很实用的推论。第一，只要 \(T_{\mathrm{comp}} \ge T_{\mathrm{comm}}\)，重叠就已经是完美的，此时再去看「重叠率」这个指标没有任何意义，因为它已经饱和了，该看的是暴露时间有没有归零。第二，重叠本身不是免费的：要把计算排到通信旁边，前提是手里有一段「不依赖这次通信结果」的计算可用，而最常见的做法就是提前发起下一次取回，于是又要多占一份显存；**绕了一圈，还是回到那三只角上**。
...
这一档的共同机制是把异步与分桶**藏进原语内部**：调用点看到的仍然是一次语义上同步的「取回完成」，用户不需要知道通信什么时候发起，框架也不打算把调度暴露出来。它换来的好处是零门槛，代价是你也不知道它在做什么。

#### torchtitan：预取留在分片原语内部

torchtitan 的参数取回由 `fully_shard` 内部的 collective 实现完成：原语内部按分片单元逐层发起 all-gather，反向时按相反的顺序发起，取回与释放的时机由 `reshard_after_forward` 这类参数调节。torchtitan 在这一层只做两件事：一是决定网格（`resolve_fsdp_mesh` 按是否启用 CP、是否启用 HSDP 拼出分片轴与复制轴），二是决定退回分片的策略（配置里的 `fsdp_reshard_after_forward` 取三个值，默认那个的语义是「不开流水线时才退回分片」，而词嵌入与输出层在权重共享或末层位置被强制为「总是退回」，代码注释解释了为什么末层要特殊对待）。**它自己一条预取链都不建，这一档的差异全部来自上游原语。**
...
这套机制有一个独立开关，而且默认是关的，不过更值得注意的是它的两个边界：一是关闭重叠时桶被退化成单桶（因为不需要分桶来创造重叠机会）；二是**分片那条路径会强制把它打开**：当分片策略包含梯度分片时，源码直接断言这个开关必须为真，理由是分片状态下梯度必须边算边规约。**所以「切分」与「重叠」在 Megatron-LM 里不是同一个决定，但到了较深的分片档位就变成强制的组合。**

还有一条与维度 2 联动的后果：打开重叠后每张卡不再持有完整的归约梯度，只持有自己的分片；需要在别处使用完整梯度的路径（例如某些权重梯度相关功能）必须显式要求走 all-reduce 而不是 reduce-scatter。

### 显式预取：框架按模块顺序提前发起取回

第二档要解决的问题和第一档其实是同一个：参数被切开那一档里，每次计算前的取回如果同步完成，就会完整暴露在关键路径上。它的做法是**由框架按模块执行顺序提前发起下一次取回**，让取回与当前模块的计算并行。所以两者的分界并不在目标上，而在「提前量由谁决定」：第一档交给分片原语的内部实现，这一档则交给框架的调度逻辑或者用户的开关。

而采用这一档的两家，在「提前量由什么决定」这件事上正好走了相反的路：一家靠执行过程中记录下来的模块顺序去推断，另一家干脆把决定权交给用户。**这个对比我觉得比它们的相似之处更有意思**，因为它说明同一档里也能长出两种完全不同的失败模式。

#### DeepSpeed：由 trace 状态机决定提前量

如维度 2 所述，DeepSpeed 的预取由 trace cache 驱动，深度硬编码为 2。**这一档的代价在它身上体现得最清楚**：只有 trace 完整时才预取，而 trace 会因为执行顺序变化而失效；预取深度又不能调，所以显存与通信的平衡点无法按机器去调。用大白话说，**它给你的是一个调好的固定档位，而不是一根可以拧的旋钮**。
...
这条链有两个很容易被忽略的门控，我第一次读的时候就没注意到：一是**开了流水线并行时它根本不建链**，源码注释说明此时权重本来就跨微批常驻、不需要预取，对应的判断直接把它短路；二是**退回分片被显式设为关时也不建链**，理由同上。所以「打开这个开关」并不等于「一定会预取」，实际是否建链取决于分片策略与是否启用流水线：**这也正是我在维度 2 里说它的字段、文档与调用点三层说法不一致的同一个问题**。
...
第三档把排布权交回给框架自己：**它主动掌握通信与计算的顺序，用异步 collective 发起、在明确的点等待**。这一档的调度自由度是三档里最高的，相应地也最依赖实现正确性：等待点只要放错一个位置，性能和正确性就会同时出问题。
...
**打开它到底改变了什么，值得按机制写清楚，因为它的三个效果各自对应着一处代价。** 一是它建了一条独立的归约流，梯度规约从默认流搬到这条流上，于是能与反向计算并行；二是它为规约桶做双缓冲并翻转缓冲下标，让「本步正在规约的桶」和「正在往里写梯度的桶」不冲突，代价是峰值显存多出一份桶；三是非本 rank 分区的梯度会被延迟清除（交给一个「上一步已规约梯度」的列表承载），并在下一次规约前做一次显式同步。**关闭它时，这三件事全部退回当前流串行执行**：拷贝与规约都落在默认流上，峰值显存少一份桶，但通信完整暴露出来。顺带一提，这条路径上还有一个同构的硬编码上限，在途的参数规约事件数同样写死为 2。
...
参数也切的那一档走的是另一条解析路径，完全不受这段检查影响。**这个差别不是抠字眼，因为 MoE 与 ZeRO-3 在 DeepSpeed 里是硬互斥的**（见维度 9）：一个想同时用流水线、专家并行和最强状态切分的用户，最终能拿到什么组合，就取决于这段分支的边界画在哪里。
...
这一档的共同机制和前面三种都不一样：它不再由框架手写缓冲与流，而是**把通信切碎这件事交给编译器，由编译机制决定通信与计算怎么交错**。
...
**都写着「支持通信重叠」，为什么调起来手感完全不同？** 因为决定重叠的那只手不是同一只。
...
带着这个上界继续往下问，就得到第四个设计问题：**给定 \(p\) 级与 \(m\) 个微批，谁来决定每个时间槽里哪一级做什么？** 我原本以为这纯粹是个调度算法问题，读下来才发现它同时牵动三件事：气泡率、同时在途的微批数（因此间接影响激活显存），以及一处容易被低估的复杂度，也就是可选的调度数量本身：用户要做选择，而框架通常不给选择标准。
...
我把这两件事放在同一节里看，并不是为了省篇幅，而是因为它们本来就是同一枚硬币的两面：**调度数量的多少本身就是一个设计选择**，它决定了用户的调优空间有多大，也决定了配错的概率有多高。
...
**「没有选择」在这里有实际含义，不只是省事**：用户的调优空间只剩下 \(m\) 与 \(p\) 两个数，气泡率只能靠加大 \(m\) 去摊薄。好处是不会配错，代价是当层数无法被 \(p\) 整除、或者激活显存不足以支撑更大的 \(m\) 时，你没有别的路可走：**这一类「没有旋钮」的设计，平时看不出代价，卡住的时候才发现无处可调**。

#### Megatron-LM：三个调度函数，但没有全前向再全反向

Megatron-LM 有**恰好三个**调度入口：无流水线、逐微批交替、交错式。全前向再全反向与双向都不在这三个里，代码与文档里也找不到任何气泡率公式（`bubble` 这个词只出现在散文注释里）。**所以「Megatron 支持 GPipe」这句话是不成立的**，而这条信息真正的价值在于：如果你需要靠全前向再全反向去换更大的稳态区域，或者靠双向调度把气泡压下去，在 Megatron-LM 上只能自己动手实现调度。
...
把这处偏差逐条列出来，我认为是这一档里最实用的一条信息，因为它能省掉一次排查：可解析的是逐微批交替与全前向再全反向这两个；文档里的交错式下划线写法会抛错，真正能解析的是去掉下划线的小写形式或驼峰式；文档里的另外几个名字同样抛错，其中一个（深度优先）在上游的候选清单里根本不存在；而文档里那个「V 形调度」，在上游的真实名字是零气泡的另一个字面值。所以**配置文档里七个值中能直接照抄的只有两个**，其余要么是语法错，要么是名字错；要确认真实可用值，应当去读上游的调度类清单，或者看仓内示例配方实际写了什么。


### 交错式：每卡多个层段

流水线这个词本身的隐喻在这里很有用：**它就像一条装配线，开机时要先把它灌满，收工前末段会先空掉，而中间那段才是满负荷运转的稳态。** 气泡说的就是灌满与空掉这两段占总时间的比例。

这一档的共同机制是**让每张卡持有多个不连续的层段**，于是同一张卡上可以交错推进 \(v\) 倍的微批，填充深度不变而稳态区被拉长。既然机制相同，两家之间的差别就落在了另一个地方：**谁来保证层段的切分是对的**。

#### Megatron-LM：把调度做成一张显式的表

Megatron-LM 的交错式调度先把调度表算出来，再按表执行：调度表的生成函数接收微批数、模型段数与每个虚拟流水线阶段包含的微批组大小，输出一张「哪个微批在哪一级的哪个槽位执行什么动作」的表。**把调度变成数据而不是控制流，在我看来带来两个很实际的后果**：一是调度变得可检查，你能在执行前把它打印出来看；二是它变得可预测，执行顺序固定，于是与激活重算、确定性模式的协作都更容易做对。

#### torchtitan 与 AutoModel：解析字符串，把切分交给配置

torchtitan 的 `pipeline_parallel_schedule` 是一个字符串，直接交给上游 PyTorch 的调度类解析函数，**它自己不做调度，只负责三件事**：把字符串解析掉、检查 stage 数约束（V 形调度要求每 rank 两个 stage，源码里有断言）、按每 stage 层数把模型切好再交给上游。
...
这一档的共同机制是**把反向计算拆成两段**。标准反向里，算某一层的输入梯度与权重梯度是连在一起的两件事，而零气泡把权重梯度那一段单独推迟出去，让它填到流水线原本空闲的槽位里，代价是这份权重梯度必须先暂存起来、稍后再执行。
...
这个差别我想特别指出来，因为它反映的是两种不同的产品形态：torchtitan 把零气泡做成「用户从清单里挑一个」，pith-train 把它做成「引擎内部的既定行为，可以开关，但不会改变调度的身份」。**前者给用户选择权，后者给用户一个已经调好的组合**，而这与两家的整体定位是吻合的：一个是通用框架，一个是 MoE 专用的紧凑框架。

### 双向调度：两个方向互填空洞

这一档要解决的是同一个气泡，但换了个思路：**让两个方向的前向与反向互相填空。** 单向流水线的气泡本来来自填充与排空，可如果同时有两个方向的微批在流水线上跑，一个方向的排空阶段就恰好能填上另一个方向的填充阶段。
...
但我在维度 3 里已经提到的一件事会限制它的收益：**它的点对点通信仍然是同步的**，请求攒批提交之后要逐个等待完成。所以这里的双向重叠**重叠的是两个方向的计算**，跨级 P2P 的传输时间依旧暴露在关键路径上。**换句话说，气泡被填掉了，但每一级之间的传递延迟没有被藏起来**：这也是它的收益上限所在。
...
**同样叫「流水线调度」，五家给的选项数量差了一个量级，这正常吗？** 数量的差别本身不是重点，重点是这些调度从哪里来、以及谁来保证前置约束。
...
维度 4 结尾把微批数的上界推给了激活显存，这笔账早晚要算，所以这一维度就来算它。**它的基本设计问题是：哪些中间张量被保存下来，哪些在反向时重新计算。** 这就是那个经典交易的具体形态，也就是用算力换显存；而我关心的是这笔交易的汇率在不同序列长度下会怎么变，因为后面你会看到，它并不是一个常数。

### 代价从哪来

要把这笔汇率算清楚，得先知道钱花在哪。以一个 batch 乘序列长度乘隐藏维的输入为例，一个 transformer 层在前向中会产生一批中间张量（QKV 投影输出、注意力输出、MLP 中间态等），粗略地说是 16 个「batch 乘序列乘隐藏维」大小的张量；除此之外还有**注意力矩阵**本身，它的大小是 batch 乘 head 数乘序列长度的平方。
...
**注意力矩阵是全部线性项的 4 倍**，这个 4 倍是后面所有结论的来源。一个 32 层模型若把这些全存下来，单是激活就要 160 GiB，而这还没算梯度、优化器状态和参数：**长上下文训练真正的瓶颈在这里，不在参数量上**。

知道钱花在哪之后，就能算重算要付多少。重算的算力代价可以按每 token 的前向与反向 FLOPs 估出来：前向约 \(2N\)、反向约 \(4N\)，合计 \(6N\)（\(N\) 为参数量）。**若把每层的前向整段重跑一遍**，就要额外付出一个 \(2N\)，总代价变成 \(8N/6N\)，也就是 **+33.3%**；这个数字的好处是它**与序列长度无关**，是个可以照抄的常数。

这个交易我想用一个更熟悉的场景来记：**重算就是「上课时把每一步都抄下来，还是只记标题、考试时自己推一遍」。** 全存相当于抄满一整本笔记，本子很重但考试时不用动脑；全量重算相当于只记标题，本子薄了，可每道题都得重推；选择性重算则介于两者之间，也就是**只把最难推的那部分抄下来**：而这恰好就是后面要算的那笔账。
...
**所以这两档真正的框架差异并不在算法上（算法都叫选择性重算），而在「可配粒度」与「重算单元由谁指定」这两件事上**，后者决定了用户需要对这个模型了解到多深。

#### Megatron-LM：二维配置，粒度与方法分开

Megatron-LM 的配置是二维的：粒度取 `full` 或 `selective`（选项恰好是这两个），方法取 `uniform` 或 `block`，名义上有四种组合。入口是核心重算模块里的一个模块级函数，而选择性那条路径走的是一个不物化输出的检查点包装器。**把粒度和方法拆成两个正交维度，好处是用户能组合；坏处是组合出来的四种里，有的差别其实很小，这一点只能靠读代码判断。**
...
**这就是「连续参数」与「离散档位」的真实区别**：内存预算档不是在几种静态策略里挑一个，而是把预算整个交给编译器去分配，所以它算得上是维度 4 留下的那个问题的半步答案。但我得把话说完整：**它管的是重算粒度，不是微批数**，而维度 4 卡住我们的那个上界恰恰在微批数上。
...
pith-train 全仓没有使用标准的检查点包装器，所以**不能说它「支持激活重算」**，我第一次检索到这里时还以为自己搜错了目录。它走的是第三条路：把重算这件事整个下沉到 kernel 内部，用三样替代手段来省激活。
...
它与其余四家的差异不是「做得少」，而是**做在别的层**：别人在模块边界上决定存不存激活，它在 kernel 内部决定重算什么。用一句大白话说，**别人把选择权交给用户，它把选择权交给了写 kernel 的人**。代价也很清楚：每引入一个新算子，就要为它单独写一个带反向重算的融合 kernel；**这是一笔把复杂度从配置面转移到 kernel 开发上的交易**，而它只在一个算子集合相对固定的紧凑框架里才划得来。

### 同维度内的比较

**都叫激活重算，为什么接入成本差这么多？** 因为「重算哪一个单元」这件事，有的框架替你决定，有的要你逐模块列出来。
...
前四个维度算的都是「多少字节」和「多少时间」，而这一维度要算的是另一种代价，也就是**精度**。它特殊在不可加：多花时间你还能接受，结果不对就没法接受了。
...
Megatron-LM 把主参数做成了一个强制项。优化器配置里主参数与主梯度的 dtype 字段默认是 fp32，而且在**未启用精度感知优化器时被断言强制住**：只要这个开关是关的，四个 dtype 字段就必须保持 fp32，否则配置校验直接失败。也就是说，它的每参数字节账里那份 fp32 参数是**显式存在**的，不是推导出来的。

这里可以用一个记账的类比来理解为什么「权威副本」这个概念绕不开：**优化器更新的那一步就像记账，用几位有效数字决定了你能不能记下一笔很小的变动。** 参数用低精度存储、却要用高精度去累加更新，本质上就是在低精度的账本上做高精度的记账，所以必须额外留一份高精度副本。

**只有打开精度感知优化器之后，这四个字段才变成真正的旋钮**，主梯度、主参数、一阶矩、二阶矩各自可以指定 dtype。但这条路径的前置条件写得相当硬，值得逐条列出来：只支持 Adam 类优化器；要求某个最低版本的 Transformer Engine 提供带相应参数的融合 Adam，否则断言失败或者直接抛错；`skip_megatron_wrapping=True` 与它不兼容。**换句话说，四个 dtype 字段看着自由，能不能用先取决于你的软件栈。**
...
**当有人问你「这一步更新发生在什么精度上」，五家的答案为什么需要不同的追查路径？** 因为他们把权威副本放在了不同的地方。
...
前面几个维度处理的都是「训练进行时」的事，这一维度要处理「训练被打断之后」的事。**它的基本设计问题是：存盘格式是否与当前并行拓扑绑定，换一个并行度之后如何恢复。** 说实话，我起初把它当成纯粹的工程细节，直到意识到它实际决定了三件事：**你的集群能不能改并行度、一次故障要回退多远、以及存盘会不会拖慢训练。**
...
这一档的共同机制很直接：**每个 rank 写自己那份，元数据只描述本次的分片布局**，至于这份数据在另一种并行度下该怎么重新拼起来，它不管。

#### Megatron-LM：重分片能力取决于优化器状态的存盘类型

Megatron-LM 用分布式检查点模块的 save/load，**隐式重分片发生在加载策略里**。这里有一处我一开始也张冠李戴的地方，值得分清：决定「能不能换并行度读回」的**不是检查点格式本身**（默认用的是它自己的分布式张量格式），而是**优化器状态的存盘类型**，后者有两个取值，一个只支持数据并行维重分片，另一个在保存时于数据并行主 rank 上聚合成「每个模型参数一份同形状状态」的规范表示，从而支持任意模型并行度；后者的开关是一个默认关闭的配置项。
...
DeepSpeed 的分片检查点由各 rank 分别写自己的分片，这一点和上一家类似。它另外提供了一个 universal checkpoint 转换脚本，**但这个脚本本身并不改变 TP/PP 度数**：输出路径仍然按原始的度数值构造，真正的重分片发生在加载侧。**把转换脚本读成「一键换并行度」是这一维度里最容易犯的错**，它实际的作用只是把分片形式规范化，好让加载侧拿到足够的信息去重排。
...
这一档的共同机制是把「怎么拼回去」这件事前移到格式本身：**格式里携带足够的信息，加载时由检查点库按目标网格重排**，因此存的时候不需要知道将来会用多少张卡。
...
**两家在这一档下的分岔有两条**：一是档位的粒度不同（torchtitan 三档，AutoModel 两个布尔量），二是对版本门槛的处理方式不同（torchtitan 没有对应检查，AutoModel 明确检查并按版本降级）。**后一条我认为更值得注意**：静默禁用异步意味着存盘会突然从后台变成阻塞，而训练脚本对此不会有任何感知：**这类「降级但不报错」的行为，恰恰是最难排查的一类性能问题**。
...
**它有一个实现细节值得单独指出来**：从分片张量里取出本地专家数据的那个函数**不触发聚合通信**，它直接读本地张量并按网格算出偏移量，于是保存时每个数据并行 rank 只写自己拥有的那些专家。**这是一个很实际的设计选择**：存盘时不需要先把所有专家权重聚合起来，写盘带宽与显存峰值都不会随专家数膨胀。代价是映射规则得由框架自己维护，新增模型结构时要同步维护这套名字规则。

### 持久化与容错：方法分类

格式解决的是「能不能读回」，持久化解决的是「什么时候写」，而容错解决的是「坏了怎么办」。这三件事经常被混为一谈，但它们的难度差了一个量级：**异步保存只是一个 IO 调度问题，容错要解决的却是「并行度变了之后，剩下的 rank 如何接管死去 rank 的状态」，而后者恰好以前者的重分片能力为前提。**
...
**换一批卡之后旧检查点还能不能读，这件事由什么决定？** 不是由检查点是不是分布式格式决定，而是由默认路径愿意承诺多少重分片能力决定。
...
前面七个维度讨论的都是框架**怎么跑**，而这一维度要讨论框架**怎么被接上**。**它的基本设计问题是：一个新的模型架构与新的并行组合以什么形式交给框架，其中哪些决定由框架做、哪些必须由用户写下来。** 换成更具体的问法就是三件事：权重从哪来、切分意图写在哪、配置能不能被静态工具检查。
...
**这与 torchtitan 构成一组很干净的对照**：AutoModel 这边接入方要写的代码最少，一个注册点就够了，代价是切分意图不由模型作者声明，而由框架的策略选择与配置项决定，于是接入方对「这个模型会被怎么切」的控制更弱。**少写代码和多拿控制权，在这里是一对真实的取舍，不是可以同时拿到的两样东西。**

### 权重从哪里来：与注册机制无关的第二条接入面

把这一维度想成「新住户怎么进小区」会直观一些：**有的小区要你去物业登记（注册表），有的小区给第三方发门禁卡（入口点），还有的小区根本没有门禁、你自己配钥匙（模型定义权交给用户）。** 差别不在谁更高级，而在登记之后你还能不能改户型。

注册机制解决的是「模型类从哪来」，而权重加载解决的是「参数从哪来」。**这两件事的成本来源不同，而且可以独立选择**：一个框架完全可能接入很简单、却要求你先做一次离线转换，也完全可能接入要写很多文件、但支持原地加载。
...
Megatron-LM 既没有模型注册表，也没有名为模型规格的对象（全仓检索零命中）。它的接入方式是写一个模型配置的子类，其中构建器是一个 `ClassVar[str]` 的点分路径字符串，框架用 `importlib` 把它解析成构建器类，而各子模块从哪里导入则由模块规格描述对象给出。**所以它的扩展点是「给一个类加一个点分路径」，而不是往注册表里加一条记录。** 这里有一处表述要避免过宽：仓库内部确实有别的注册表（视觉编码器与某个 RL 组件各有一个映射表），准确的说法是「**模型**这一层没有注册表」。
...
**同一个新架构，接进五家分别要动多少东西？** 差别不在代码行数，而在成本落在谁身上、以及是一次性投入还是持续投入。
...
前面那些维度都可以在稠密模型上讨论，而这一维度是第一个**只有稀疏模型才会遇到**的问题。**它的基本设计问题是：专家落在哪些设备上，token 如何在设备之间被派发出去、又如何在算完之后收回来。** MoE 给训练框架带来三个新问题：专家的计算是**稀疏且不规则触发**的（每个 token 只被路由到少数几个专家，因此每个专家收到的 token 数事先不知道，取决于 router 的输出）；token 要**跨设备搬运**（这是 all-to-all，与前面所有集合通信的形状都不同）；负载**天然不均衡**（router 会自我强化：被选得越多的专家训得越好、越容易被再选，需要辅助损失或容量因子纠偏）。
...
先用一个分拣中心的类比把这件事的形态说清楚：**dispatcher 相当于把包裹按目的地重新装车（all-to-all），专家计算相当于各个站点分别处理，combine 相当于把处理完的包裹送回原寄件人。** 有意思的是，包裹总量其实不大，真正让分拣中心难受的是「每一层都要全员到齐一次」这件事。

那么「EP 让通信变贵」这句话还成立吗？**成立，但原因不在通信量上，而在同步结构与可预测性上。** 每一层、每一次前向，EP 都要做 dispatch 与 combine 两次全局 all-to-all，而每一次都要求全员到齐；可另一方面，dense 模型的 all-gather 同样要搬全部参数（就是维度 3 里那个 \(3P\)），量级一点都不小。**两种通信真正的差别在于：dense 的 all-gather 是规整的、可预测的、能提前预取的；EP 的 all-to-all 是数据依赖的，必须先算完 router 才知道要把哪些 token 发到哪里。用一句大白话说，不可预测的通信比大通信更难藏。**
...
DeepSpeed 是唯一落在这一档的框架。不过在往下讲它的四条限制之前，**我要先纠正一个我自己一开始也搞过头的前提：它并不是「没有跨设备路由」**。它有一个真正的 all-to-all 自动微分函数（前向直接调 all-to-all-single，反向自反），也按容量因子做 token 丢弃，专家本身是每卡本地存放而不复制的。**所以它与另外四家的差别，只在专家计算的实现方式这一处。**

它的限制需要整组写出才能看清后果：

1. **没有独立的 MoE 配置块**：运行时配置与常量里检索 `moe` 零命中，MoE 通过层构造参数配置。
2. **训练侧没有 grouped GEMM**：专家在实现里是**每个专家一次 Python 循环调用**的，这与另外四家走 grouped GEMM 是本质差别，后果是每张卡上本地专家个数的 kernel 要串行启动。
...
**这条优化目前只有 pith-train 在做**，其余四家的力气都花在换更好的派发后端上，这正好是同一维度里两种不同的优化方向（一种减少要搬的数据，一种让搬运更快），我在本节末尾会把它们并起来比较。
...
**都是「支持专家并行」，为什么默认配置下的性能曲线不一样？** 因为稀疏收益在有的框架里是默认，在有的框架里要先显式打开。
...
前面九个维度讨论的都是「怎么跑得更快或更省」，而这一维度要问的是**怎么知道它跑对了**。之所以单独成维，是因为分布式训练的正确性有两个无法回避的现实：并行会改变归约顺序与 kernel 形状，因此结果与单卡不会逐位相同；而集群规模越大，静默的硬件或通信错误越难靠肉眼发现。
...
这一档的共同机制是**打开框架的确定性算法开关，并把通信库的归约算法与协议一起固定下来**。它有一个共同的陷阱需要提前知道：**这些环境变量必须在进程组初始化之前设定**，因为它们影响的是通信库初始化时读到的配置，设晚了就不生效。
...
**与 Megatron-LM 的对照非常清楚**：两家都在固定通信库行为，但一家明确拒绝 Tree，另一家反而把归约算法固定为树形（批次不变路径里固定为 `allreduce:tree`）。这一档的验收标准可以用一句制造业的话来概括：**同一张图纸、同一条流水线跑两遍，出来的必须是同一件产品。** 差别在于「同一件」这三个字要求到什么程度：只要求两次运行一致，还是要求同一样品与谁同批也一致。

**这处分歧不是谁写错了，而是两种不同的固定策略**：固定到 Ring 是从「归约顺序本身可验证」出发，固定到 Tree 则是为了配合批次不变算子包的整体行为。**它说明「确定性」在这个层级上从来不是一个单点开关，而是一组必须与算子实现共同选定的配置**：这一点我原本以为只是文档措辞问题，读源码才意识到是设计上的必然。
...
**「可复现」这三个字，五家承诺的到底是什么？** 口径从「不做承诺」到「与批次组成无关」依次增强，而承诺越强、能用的优化越少。
...
逐维度记十个问题很容易忘，所以我想换一个角度收尾：把每个框架当成**一组前后自洽的取舍**来看，比记十张表更容易得到可用的判断。下面这五段就是我读完源码之后对每一家的总结，它们不是优点清单，而是「它有意识地选择了什么、因此必须放弃什么」。
...
**AutoModel 把接入成本压到最低，代价是底层旋钮的暴露不一致。**（这一条我个人感受最深，因为它的入口确实好用到让人放松警惕。） 它的第三方模型注册走 entry point，HF 权重可以在同一个调用里加载并接管分布式，这是五家里接入成本最低的一条路。但它的精度策略由模型加载精度推导、预取开关的文档与字段与调用点三层说法不一致、调度文档列出的值多数不能解析。**接入快与调优可控在这个框架里是一对真实的取舍**，而不是可以同时拿到的东西。
