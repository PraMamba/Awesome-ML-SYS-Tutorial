...
- **维护活跃度我只是给了可复算的计数，没有做趋势预测。** 本文的做法是「目录 + 12 个月窗口 + commit 计数」（见 §7），因为「最近 90 天有没有 commit」这个判据会漏掉「快照冻结」这种情况：DeepSpeed 的本地 checkout 停在 2026-06-04、近 90 天 0 个 commit，但拉长到 12 个月它有 262 个 commit（月均 17–45 个），直到 2026-05 都正常。**把快照冻结读成项目废弃，是这类调研最常见的错误。**
...
按「目录 + 12 个月窗口 + commit 计数」给出，全部可在本文引用的 checkout 上复算（窗口起点：365 天前 = 2025-09-12，90 天前 = 2026-06-14，统计时点 2026-09-12）：

| 后端                | 最后提交日期         | 12 个月 commit 数 | 90 天 commit 数 | 说明                                                                        |
| ------------------- | -------------------- | ----------------- | --------------- | --------------------------------------------------------------------------- |
| Megatron-LM         | 2026-09-12           | 2611              | 746             | 本地 HEAD 那个提交是文档提交                                                |
| **DeepSpeed** | **2026-06-04** | **262**     | **0**     | ⚠️**快照冻结**，不是项目废弃（月均 17–45 个，直到 2026-05 都正常） |
| torchtitan          | 2026-09-12           | 1338              | 473             | HEAD 那个提交只是工具配置                                                   |
| NVIDIA AutoModel    | 2026-09-12           | 1733              | 634             | 同上                                                                        |
| pith-train          | 2026-09-09           | 83                | 29              | 仓库不满一年，所以 12 个月计数等于总提交数                                  |
