...
- [Translating the Memory Wall into a Partitioning Bill: A Cross-Project Survey of Six Training Backends](./rlhf/training-backend-landscape/readme.md): A survey of six large-model training backends along nine dimensions - parallelism combinations, the per-parameter byte budget, computation-communication overlap, pipeline bubbles, activation memory versus recomputation, precision strategy and numerical consistency, checkpoint resharding and fault tolerance, MoE with expert parallelism, and the model-integration surface. Numbers come from documentation or from arithmetic the reader can redo, and documentation-versus-source conflicts are recorded in a dedicated section.
