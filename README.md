...
- [How Agent Harnesses Attach to RL: The Real Architecture of Open-Source Infrastructure in 2026](./rlhf/agent-harness-rl/readme.md): A source-level survey of attaching an off-the-shelf agent harness to RL training at the LLM API boundary - the ownership models and integration boundaries, the call tuple one model call has to persist, why re-tokenization drift breaks log pi, ten candidate implementations plus their counterexamples, the post-capture invariants (compaction, reward grouping, policy version, retry dedup), and an acceptance checklist for choosing an integration route. Static audit only: every reference is pinned to a reviewed commit, and no GPU training was run.
- [Keeping Tokens Flowing: A Cross-Project Survey of Asynchronous RL Training Infrastructure](./rlhf/async-rl-training-landscape/readme.md): A survey of asynchronous training infrastructure across 16 open-source RL/post-training frameworks read at pinned commits - the seven stages of the synchronous loop and the KV-cache-bound cost of generation, the four design questions that appear once generation and training are split (orchestration, data path and admission, weight return and its visibility boundary, and the consistency of stale data and in-flight requests), and seven orthogonal design dimensions. Each dimension moves from the design question to a method taxonomy with a framework map, then compares how the frameworks on each method actually execute and where their implementations diverge, with every claim pinned to a repository, commit and file line, and closes on the couplings between dimensions. No framework is benchmarked here.
- [One Design Question, Five Engineering Answers: A Method-by-Method Survey of Five LLM Training Frameworks](./rlhf/training-backend-landscape/readme.md): a ten-dimension, method-by-method survey of Megatron-LM, DeepSpeed, torchtitan, NVIDIA AutoModel and pith-train - deriving the parallel topology, distributing state and its per-GPU cost, timing and overlapping communication, laying out microbatches on the pipeline, saving versus recomputing intermediates, master-weight copies and the precision budget, checkpoint format and resharding, model onboarding and the configuration surface, expert placement and token routing, and verifying the equivalence of parallel execution. Each dimension classifies the methods, places the frameworks, then compares their implementations within each method against pinned local source. Chinese only.
...
- [FSDP2 Systematic Tutorial](./torch/fsdp2/readme.md): A 7-stage systematic tutorial from distributed basics to combined parallelism (FSDP+TP), with 22 runnable scripts and detailed documentation. Requires PyTorch >= 2.4.
- [DeepEP Deep Dive: From MoE Token Movement to Topology, Memory Layout and Overlap](./torch/deepep/deep-dive.md): From tokens, token-rank pairs and token-expert pairs to the dispatch/combine matrix form and recomputable communication volume; the V1 normal and low-latency paths, the V2 ElasticBuffer and EPHandle interfaces, SM and QP estimation, and the Megatron-LM, SGLang, verl and vLLM integration boundaries. Ships with two CPU-only teaching implementations (single process and four-rank Gloo) and per-formula, per-image verification notes; all source links are pinned to a fixed commit.
- [DualPipe Deep Dive: Where Communication Hides Once the Pipeline Is Full](./torch/dualpipe/deep-dive.md): Separates the bubble at the schedule boundary from the communication wait inside a stage, then follows GPipe, 1F1B, Zero Bubble, DualPipe and DualPipeV on one timeline, with the steady-state pairing derived index by index, a recomputed bubble and memory accounting, and the source facts of the official 440-line skeleton plus the PyTorch and Megatron layers. CPU-only schedule simulation and no GPU measurement.
