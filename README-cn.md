...
- [让 Token 持续流动：16 个开源 RL 框架的异步训练基础设施横向调研](./rlhf/async-rl-training-landscape/readme.md)：把 16 个仍在活跃维护的 RL / post-training 框架固定 commit 逐个读源码后的横向调研——同步 RL 循环的七个阶段与 decode 被 KV cache 读带宽锁死的成本、生成与训练拆开之后必须显式回答的四类设计问题（编排与控制、数据通路与准入、权重回传与生效、旧数据与在途状态的一致性），以及七个正交的设计维度。每个维度先回答一个基本设计问题、再归纳方法并给出框架映射，然后逐方法比较各框架的实际执行机制与实现分岔，每条断言落到「仓库 + commit + 文件行号」，最后收束维度之间的耦合与三个仍然开放的问题。本文不做任何框架实测。
- [同一个设计问题，五种工程答案：五个大模型训练框架的逐方法横向调研](./rlhf/training-backend-landscape/readme.md)：沿十个基本设计决策拆解 Megatron-LM、DeepSpeed、torchtitan、NVIDIA AutoModel 与 pith-train——并行拓扑的推导、状态的分布与每卡静态开销、通信的发起时机与重叠、微批在流水线上的排布、中间结果的保存与重算、主权重副本与精度预算、检查点的格式与重分片、模型接入与配置面、专家的分布与 token 路由，以及并行执行的等价性验证。每个维度先归纳方法并给分类表，再按方法逐个框架写实现，最后收拢同方法内的分岔；事实以本地源码固定 commit 为依据。
